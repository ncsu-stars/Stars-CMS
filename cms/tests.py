...
class MemberTest(SimpleTestCase):
    def setUp(self):
        ...
        Member.objects.create(user=User.objects.create(username="user1",
                                                       first_name="Random",
                                                       last_name="User"),
                              group="Undergraduate", classification="freshman",
    ...
    def test_get_coordinated_projects(self):
        ...
        Project.objects.create(title="Project1", description="Blah",
                               status=1, category=0,
                               year=settings.CURRENT_YEAR)
        expectedProject = Project.objects.get(title="Project1")
        ProjectMember.objects.create(project=expectedProject, member=member1,
    ...
    def test_get_possible_project_members(self):
        Member.objects.create(user=User.objects.create(username="user2"),
                              group="Undergraduate", classification="freshman",
        ...
        self.assertIsInstance(actualProjectMembers,
    ...
    def test_correct_status(self):
        Member.objects.create(user=User.objects.create(username="user2",
                              ...
                              group="Undergraduate", classification="freshman",
                              status=Member.STATUS_ARCHIVED)
        Member.objects.create(user=User.objects.create(username="user3",
                                                       first_name="Random",
                                                       last_name="User"),
                              group="Undergraduate", classification="freshman",
    ...
    def test_member_creation(self):
        Member.objects.create(user=User.objects.create(username="user2"),
                              classification="freshman", hometown="hometown",
                              interests="", homepage="", blurb="",
                              status=Member.STATUS_ARCHIVED)
        Member.objects.create(user=User.objects.create(username="user3"),
                              group="Undergraduate", hometown="hometown",
                              interests="", homepage="", blurb="",
                              status=Member.STATUS_ARCHIVED)
        Member.objects.create(user=User.objects.create(username="user4"),
                              group="Undergraduate", classification="freshman",
                              interests="", homepage="", blurb="",
                              status=Member.STATUS_ARCHIVED)
        Member.objects.create(user=User.objects.create(username="user5"),
                              group="Undergraduate", hometown="hometown",
                              classification="freshman", homepage="", blurb="",
                              status=Member.STATUS_ARCHIVED)
        Member.objects.create(user=User.objects.create(username="user6"),
                              group="Undergraduate", hometown="hometown",
                              interests="", classification="freshman",
                              blurb="", status=Member.STATUS_ARCHIVED)
        Member.objects.create(user=User.objects.create(username="user7"),
                              group="Undergraduate", hometown="hometown",
                              interests="", homepage="",
                              classification="freshman",
                              status=Member.STATUS_ARCHIVED)
        with self.assertRaisesRegexp(IntegrityError,
                                     "NOT NULL.*cms_member.status"):
            Member.objects.create(user=User.objects.create(username="user8"),
                                  group="Undergraduate", hometown="hometown",
                                  interests="", homepage="", blurb="",
                                  classification="freshman")
        with self.assertRaisesRegexp(IntegrityError,
                                     "NOT NULL.*cms_member.user_id"):
            Member.objects.create(group="Undergraduate",
                                  classification="freshman",
                                  hometown="hometown", interests="",
                                  homepage="", blurb="",
                                  status=Member.STATUS_ARCHIVED)
        with self.assertRaisesRegexp(IntegrityError,
                                     "UNIQUE.*auth_user.username"):
            Member.objects.create(user=User.objects.create(username="user7"),
                                  group="Undergraduate", hometown="hometown",
                                  interests="", homepage="",
                                  classification="freshman",
...
class ProjectTest(SimpleTestCase):
    def setUp(self):
        self.tearDown()
        Project.objects.create(title="Project1",
                               status=Project.STATUS_ACTIVE,
                               year=cms.academic_year(cms.get_current_time()))
    ...
    def test_is_member_coordinator_no_members(self):
        Member.objects.create(status=Member.STATUS_ACTIVE,
    ...
    def test_get_status(self):
        Project.objects.create(title="Project2",
                               ...
                               year=cms.academic_year(cms.get_current_time()))
        Project.objects.create(title="Project3", status=Project.STATUS_EMPTY,
                               year=cms.academic_year(cms.get_current_time()))
    ...
    def test_get_category(self):
        Project.objects.create(status=Project.STATUS_ARCHIVED,
                               category=Project.CATEGORY_INTERNSHIP,
                               year=cms.academic_year(cms.get_current_time()))
        Project.objects.create(status=Project.STATUS_EMPTY,
                               category=Project.CATEGORY_ORGANIZATIONAL,
                               year=cms.academic_year(cms.get_current_time()))
        Project.objects.create(status=Project.STATUS_ARCHIVED,
                               category=Project.CATEGORY_OUTREACH,
                               year=cms.academic_year(cms.get_current_time()))
        Project.objects.create(status=Project.STATUS_EMPTY,
                               category=Project.CATEGORY_SERVICE,
                               year=cms.academic_year(cms.get_current_time()))
        Project.objects.create(status=Project.STATUS_ARCHIVED,
                               category=Project.CATEGORY_RESEARCH,
                               year=cms.academic_year(cms.get_current_time()))
...
class ProjectMemberTest(SimpleTestCase):
    def setUp(self):
        self.tearDown()
        Member.objects.create(user=User.objects.create(username="1",
                                                       first_name="First",
                                                       last_name="Last"),
                              status=Member.STATUS_ACTIVE)
        Project.objects.create(title="Project2",
                               status=Project.STATUS_ARCHIVED,
                               year=cms.academic_year(cms.get_current_time()))
        ProjectMember.objects.create(project=Project.objects.get(pk=1),
                                     member=Member.objects.get(pk=1))
        ProjectMember.objects.create(project=Project.objects.get(pk=1),
                                     volunteer_name="Volunteer")

    def test_get_full_name_from_volunteer(self):
        self.assertEqual(u"Volunteer",
                         ProjectMember.objects.get(pk=2).get_full_name())
    ...
    def test_get_full_name_from_member(self):
        ...
        self.assertEqual(User.objects.get(pk=1).get_full_name(),
                         project_member.get_full_name())
...
class NewsTest(SimpleTestCase):
    ...
    def setUp(self):
        News.objects.create(title="News1!",
...
class PageTest(SimpleTestCase):
    ...
    def setUp(self):
        Page.objects.create(title="TitleForThisPage",
                            content="Lots of content is awesome.",
                            slug="thisismyslug")
...
class SponsorTest(SimpleTestCase):
    ...
    def setUp(self):
        mock_image = SimpleUploadedFile('test/file.jpg',
                                        "Image contents",
