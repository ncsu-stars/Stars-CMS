...
def make_member_image_name(instance, filename):
    ...
    return os.path.join(MEMBER_IMAGE_FOLDER, str(instance.pk)
...
def make_project_image_name(instance, filename):
    ...
    return os.path.join(PROJECT_IMAGE_FOLDER, str(instance.pk)
                        + os.path.splitext(filename)[1].lower())
...
def make_sponsor_image_name(instance, filename):
    ...
    return os.path.join(SPONSOR_IMAGE_FOLDER, str(instance.pk)
                        + os.path.splitext(filename)[1].lower())
...
class Member(models.Model):
    ...
    classification = models.CharField(max_length=255, choices=CLASS_CHOICES,
                                      blank=True)
    hometown = models.CharField(max_length=255, blank=True)
    interests = models.TextField(blank=True)
    homepage = models.URLField(blank=True)
    blurb = models.TextField(blank=True)
    image = models.ImageField(upload_to=make_member_image_name,
    ...
    def get_coordinated_projects(self):
        return Project.objects.filter(pk__in=ProjectMember.objects.filter(
            member__pk=self.pk, is_coordinator=True).values_list('project__pk',
                                                                 flat=True))

    @staticmethod
    def get_possible_project_members():
        # need to allow member one year old as well since activity status is
        # predicated on project membership but you can't create a project for
        # a new year without a project coordinator hence, the one year offset
        # avoids the chicken-and-the-egg problem return
        return Member.objects.exclude(status=Member.STATUS_ARCHIVED)\
...
class Project(models.Model):
    # this project is pending "creation" by coordinator
    STATUS_EMPTY = 0
    # this project has been created and is editable by coordinators
    STATUS_ACTIVE = 1
    # this project has been archived and is frozen
    STATUS_ARCHIVED = 2
    ...
    image = models.ImageField(upload_to=make_project_image_name,
                              storage=OverwriteStorage(), blank=True)
    status = models.IntegerField(choices=STATUS_CHOICES)
    ...
    def get_absolute_url(self):
        return reverse('cms:projects_year_pk_url',
                       kwargs={'year': self.year, 'pk': self.pk})

    def is_member_coordinator(self, member):
        return ProjectMember.objects.filter(member__pk=member.pk,
                                            project__pk=self.pk,
                                            is_coordinator=True).count() != 0
...
class Page(models.Model):
    ...
    pub_front_page = models.BooleanField(default=False,
                                         verbose_name='Publish on homepage')
    pub_menu = models.BooleanField(default=False,
                                   verbose_name='Publish on top menu')
...
class BlogPost(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('cms:blog_post_url',
                       kwargs={'pk': self.author.pk, 'blog_pk': self.pk})
...
class Sponsor(models.Model):
    name = models.CharField(max_length=255)
    image = models.ImageField(upload_to=make_sponsor_image_name,
                              storage=OverwriteStorage(), blank=True)
